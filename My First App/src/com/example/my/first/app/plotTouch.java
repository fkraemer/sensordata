...
public class plotTouch implements OnTouchListener {
	...
	public plotTouch(final XYPlot plot, float ABS_X_MIN, float ABS_X_MAX,
			...
			float MIN_X_DISTANCE, float MIN_Y_DISTANCE, float MAX_Y_DISTANCE,
		...
		Thread t = new Thread() {
			...
			public void run() {
				while (!isInterrupted()) {
					try {
						sleep(200);
						long midValue = (long) (0.5 * (maxXY.x + minXY.x));
						Date date = new Date(midValue);
	...
	public boolean onTouch(View arg0, MotionEvent event) {
		// double valY = ValPixConverter.pixToVal(event.getY(0), minXY.y,
		// maxXY.y, plot.getHeight(), true);
		// double valX = ValPixConverter.pixToVal(event.getX(0), minXY.x,
		// maxXY.x, plot.getWidth(), false);
		// if (!(valX<maxXY.x && valX>minXY.x && valY<maxXY.y && valY>minXY.y))
		// return false;
	...
	private void scroll(float panX, float panY) {
		...
		float[] k = forceBorders(minXY.x, maxXY.x, minXY.x + offset, maxXY.x
				+ offset, Direction.X);

		plot.setUserDomainOrigin((plot.getDomainOrigin()).floatValue() + k[0]
				- minXY.x);
		minXY.x = k[0];
		maxXY.x = k[1];
		...
		k = forceBorders(minXY.y, maxXY.y, minXY.y + offset, maxXY.y + offset,
				Direction.Y);
		minXY.y = k[0];
		maxXY.y = k[1];
		plot.setUserRangeOrigin((plot.getRangeOrigin().longValue())
				+ ((Float) (k[0] - minXY.y)).longValue());
	 ...
	 * midPoint = new PointF(0.5f * ev.getX(0) + 0.5f * ev.getX(1), 0.5f *
	 * ev.getY(0) + 0.5f * ev.getY(1)); } else midPoint = new
